...
// const printTemplate = handlebars.compile(fs.readFileSync(path.resolve(path.join(options.website.template.path, options.website.template.print))).toString());
...
//   const fileLocation = path.resolve(path.join(options.website.directory, 'print-' + poem.meta.id + '.html'));
...
for (let poem of section.data){
  ...
  poemHtml = pretty(poemHtml, {ocd: true});
  const fileLocation = path.resolve(path.join(options.website.directory, poem.meta.id + '.html'));
  fs.writeFileSync(fileLocation, poemHtml);
}
}
...
const indexLocation = path.resolve(path.join(options.website.directory, options.website[id]));
